...
COPY oura/oura_post_to_influxdb.py /etc/oura/
RUN chmod +x /etc/oura/oura_post_to_influxdb.py
COPY oura/PAT.txt /etc/oura/
RUN pip3 install influxdb
COPY oura/oura_query.py /etc/oura/
RUN chmod +x /etc/oura/oura_query.py

# Configure cron to query for new data
COPY crontab/oura_post /etc/cron.d/
RUN chmod 0644 /etc/cron.d/oura_post &&\
    crontab /etc/cron.d/oura_post
